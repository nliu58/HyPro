...
def calculate_igm(igm_image_file, imugps_file, sensor_model_file, dem_image_file, boresight_options):
    ...
    samples = L0.shape[1]
    igm_image = np.full((3, lines, samples), fill_value=np.nan, dtype='float64')
    info = 'Line (max=%d): ' %lines
    for line in range(lines): # line loop
        if line%100 == 0:
            info += '%d ,' %(line+1)
        for sample in range(samples): # sample loop
            igm_image[0, line, sample], igm_image[1, line, sample], igm_image[2, line, sample]= \
            ray_tracing(xyz0[:, sample, line],
                        xyz1[:, sample, line],
                        L0[:, sample, line],
                        dem_image,
...
def ray_tracing(XYZ0, XYZ1, V, DEM, DEM_X0Y0, DEM_Resolution):
    ...
    cellsize_x = DEM_Resolution
    cellsize_y = -DEM_Resolution
